...
def runMakefile (toolDirectory, archiveBaseURL, llvmSourceList, \
  ...
  #--- Compile LLVM
    asObject = objectDir + "/optimized-" + source + ".s"
    rule = make.Rule (asObject, "Compiling Optimized " + source)
    rule.mDependences.append (optimizedSource)
    rule.mCommand += llvmCompiler
    rule.mCommand += llvmCompilerOptions
  ...
  #--- Assembling
    ...
    rule = make.Rule (object, "Assembling " + source + ".s")
    rule.mDependences.append (asObject)
    rule.mCommand += assembler
