...
int32_t teensy_write(unsigned char *buf, int32_t len, double timeout);
void teensy_close(void);
int32_t hard_reboot(void);

// Intel Hex File Functions
int32_t read_intel_hex(const char *filename);
int32_t ihex_bytes_within_range(int32_t begin, int32_t end);
void ihex_get_data(int32_t addr, int32_t len, unsigned char *bytes);
int32_t memory_is_blank(int32_t addr, int32_t block_size);
...
int32_t wait_for_device_to_appear = 0;
int32_t hard_reboot_device = 0;
int32_t reboot_after_programming = 1;
int32_t verbose = 0;
...
int32_t main(int32_t argc, char **argv)
{
  ...
  int32_t num, addr, r, write_size=block_size+2;
  int32_t first_block=1, waited=0;
...
libusb_device_handle * open_usb_device(int32_t vid, int32_t pid)
{

  libusb_context * usb_ctx = NULL ;
  libusb_init (& usb_ctx);
//  libusb_find_busses (usb_ctx);
//  libusb_find_devices (usb_ctx);
...
//    for (dev = bus->devices; dev; dev = dev->next) {
   ...
   for (i=0 ; i<n ; i++) {
     ...
     libusb_get_device_descriptor(dev, & desc) ;     
      ...
      #endif
      // Mac OS-X - removing this call to libusb_claim_interface() might allow
      // this to work, even though it is a clear misuse of the libusb API.
      // normally Apple's IOKit should be used on Mac OS-X
 /*    int32_t r = libusb_claim_interface(h, 0);
      if (r < 0) {
        libusb_close(h);
...
int32_t teensy_write (unsigned char *buf, int32_t len, double timeout)
{
  ...
  while (timeout > 0) {
    r = libusb_control_transfer (libusb_teensy_handle, 0x21, 9, 0x0200, 0,
      buf, len, (int32_t)(timeout * 1000.0));
    if (r >= 0) return 1;
...
int32_t hard_reboot(void)
{
  ...
  r = libusb_control_transfer (rebootor, 0x21, 9, 0x0200, 0, (uint8_t *) reboot, 6, 100);
...
IOHIDDeviceRef open_usb_device(int32_t vid, int32_t pid)
{
...
int32_t open_usb_device(int32_t vid, int32_t pid)
{
  int32_t r, fd;
  DIR *dir;
  struct dirent *d;
...
static int32_t uhid_teensy_fd = -1;
...
static int32_t end_record_seen=0;
static int32_t byte_count;
static uint32_t extended_addr = 0;
static int32_t parse_hex_line(char *line);
...
{
  int32_t addr, code, num;
        int32_t sum, len, cksum, i;
        char *ptr;
        ...
        ptr += 2;
        if ((int32_t)strlen(line) < (11 + (len * 2)) ) return 0;
