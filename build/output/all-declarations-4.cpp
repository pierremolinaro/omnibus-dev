...
void extensionMethod_generateLLVM (const GALGAS_staticlistValues_5F_listMap inObject,
                                   ...
                                   C_Compiler * inCompiler
                                   COMMA_UNUSED_LOCATION_ARGS) {
  GALGAS_bool var_first_13713 = GALGAS_bool (true) ;
  const GALGAS_staticlistValues_5F_listMap temp_0 = inObject ;
  cEnumerator_staticlistValues_5F_listMap enumerator_13749 (temp_0, kENUMERATION_UP) ;
  while (enumerator_13749.hasCurrentObject ()) {
    const enumGalgasBool test_1 = constinArgument_inUsefulStaticArrayMap.getter_hasKey (enumerator_13749.current_key (HERE) COMMA_SOURCE_FILE ("declaration-extend-static-array.galgas", 304)).boolEnum () ;
    if (kBoolTrue == test_1) {
      const enumGalgasBool test_2 = var_first_13713.boolEnum () ;
      if (kBoolTrue == test_2) {
        var_first_13713 = GALGAS_bool (false) ;
