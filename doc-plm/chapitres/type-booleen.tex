...
L'attribut \plm=@unsigned8= n'est pas prédéfini dans le langage, mais doit être déclaré dans une déclaration \plm=newUnsignedRepresentation=\index{newUnsignedRepresentation@\plm=newUnsignedRepresentation=} (voir \refSubsectionPage{DefNewUnsignedRepresentation}), qui figure normalement dans la définition de la cible. Si l'auteur de cette définition est logique, \plm=@unsigned8= représente dans le code C engendré les booléens comme des entiers non signés de 8 bits.
