...
\begin{PLM}
  public func `user `panic `service `kernel write (?off:inLeds $uint32) 
\end{PLM}
...
\begin{PLM}
  public func `user `panic `service `kernel write (?on:inLeds $uint32) 
\end{PLM}
...
\begin{PLM}
  public func `user `panic `service `kernel write (?toggle:inLeds $uint32) 
\end{PLM}
...
\begin{PLM}
func `kernel block (?!inList:ioWaitingList $taskList)
\end{PLM}
...
\begin{PLM}
func `kernel block (?onDeadline:deadline $uint32) 
\end{PLM}

Cette fonction bloque la tâche en cours jusqu'à la date \plm=deadline=. Elle est appelable en mode \plm=`kernel=, c'est-à-dire par les primitives.
...
\begin{PLM}
func `kernel block (?!inList:waitingList $taskList 
                    ?onDeadline:deadline $uint32) 
\end{PLM}

Cette fonction bloque la tâche en cours dans la liste \plm=waitingList= jusqu'à la date \plm=deadline=. Elle est appelable en mode \plm=`kernel=, c'est-à-dire par les primitives.
