...
{\bf Ligne 6.} Déclaration des types entier relatifs à la panique (\refSectionPage{typePanique}). Le premier type définit le type du code de la panique (\plm+$int32+, dont un code peut être négatif, positif, ou nul), le second le type du numéro de ligne (\plm+$uint32+, un numéro de ligne est positif). Cette déclaration est obligatoire.
