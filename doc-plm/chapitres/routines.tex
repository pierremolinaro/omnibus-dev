...
\begin{PLM}
proc goto `user @noUnusedWarning (
  ?line:inLine $uint32
  ?column:inColumn $uint8) {
}
\end{PLM}

Ceci définit la procédure \plm=goto=, appelable uniquement en mode \plm=`user=, avec deux arguments formels en entrée. L'attribut \plm+@noUnusedWarning+ signifie qu'aucune alerte n'est émise si la procédure n'est pas utilisée.
...
\subsectionLabel{Attribut \texttt{@noUnusedWarning}}{attributNoWarningIfUnused}\index{"@noUnusedWarning}

L'attribut \plm+@noUnusedWarning+ signifie qu'aucune alerte n'est émise si la procédure n'est pas utilisée.
...
\begin{PLM}
func maFonction `user @noUnusedWarning () -> $uint32 {
  result = 6
}
\end{PLM}
...
\begin{itemize}
  ...
  \item \plm=@attribut1= \plm=@attribut2= est une liste éventuellement vide d'attributs associés service ; actuellement, seul l'attribut \plm!@noUnusedWarning! est défini (\refSubsectionPage{attributNoWarningIfUnusedService}).
\end{itemize}
