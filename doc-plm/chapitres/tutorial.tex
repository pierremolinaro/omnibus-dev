...
\begin{figure}[ht]
  ...
  \footnotesize
  \begin{tikzpicture}[
      block/.style ={rectangle, draw=black, thick, fill=white, align=center, minimum height=0.6cm, minimum width=1.4cm},
      node distance=0.5cm and 1.4cm
    ]
    \node [block] (plm) {\tt f{}ichier.plm} ;
    ...
    \node [block] (clean) [left=of link] {\tt clean.py};
    \node [block] (build) [left=of make] {\tt build.py};
    \node [block] (run) [left=of plms] {\tt run.py};
    ...
    \draw [-stealth, thick] (plm.east) -- +(0.7, 0) |- (build) ;
    \draw [-stealth, thick] (plm.east) -- +(0.7, 0) |- (run) ;
    \draw [-stealth, thick] (plm.east) -- +(0.7, 0) |- (clean) ;
...
\end{tabular}
\caption{Scripts Python engendrés dans le répertoire \texttt{ws} par la compilation}
...
\end{tabular}
\caption{Fichiers engendrés dans le répertoire \texttt{ws/sources} par la compilation}
