...
PLM définit aussi le type \plm!LiteralInt!, qui est le type de toute constante entière littérale. Mais ce type est aussi applicable à des constantes entières \emph{statiques}, c'est-à-dire dont la valeur est calculée à la compilation. Ce type est décrit au \refChapterPage{chapitreTypeEntierStatique}.
...
Une constante littérale entière a pour type \plm!LiteralInt!, or ce type n'est pas acceptable pour une variable. Par exemple, si on écrit :
\begin{PLM}
...
L'annotation de type après le mot-clé \plm=convert= est optionnel~: par défaut, le type est inféré par le contexte. Par exemple, on peut écrire~:
\begin{PLM}
let s Int8 = ...
...
L'annotation de type après le mot-clé \plm=truncate= est optionnel~: par défaut, le type est inféré par le contexte. Par exemple, on peut écrire~:
\begin{PLM}
let s Int8 = -10
...
L'accesseur \plm=leadingZeroCount= est implémenté pour tout type entier. Il retourne le nombre de bits à zéro à partir du bit le plus significatif. Par exemple~:
...
Attention, une tranche \plm=!1:= signifie que l'expression doit être un entier non signé sur un bit, et non pas un booléen. Si l'on veut initialiser une tranche de 1 bit à partir d'un booléen, il faut utiliser le sélecteur particulier \plm=!b:=. Par exemple~:
\begin{PLM}
