...
Une structure peut définir des \emph{primitives}. Une primitive s'exécute en mode \plm=`kernel=\index{`kernel}, c'est-à-dire interruptions matérielles masquées ; les fonctions de l'exécutif pouvant bloquer ou libérer des tâches sont accessibles dans ce mode. Un exemple de service est la primitive \plm=P= du sémaphore de Dijkstra (\refSectionPage{semaphore}).
