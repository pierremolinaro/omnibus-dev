...
\begin{PLM}
let SIZE = 20
newtype $monTypeTableau : $uint32 [SIZE]
\end{PLM}


\sectionLabel{Construction d'un tableau}{ConstructionTableau}

L'expression \plm=$monTypeTableau (!repeated:exp)= est une instance du type \plm=$monTypeTableau=, dont tous les éléments sont initialisés avec la valeur de \plm=exp=. Cette expression permet de fixer la valeur initiale d'une variable de ce type.
...
L'expression \plm=$monTypeTableau.count= renvoie la taille du tableau, sous la forme d'un entier statique (c'est-à-dire de type \plm=$staticInt=, voir \refChapterPage{chapitreTypeEntierStatique}). Ou peut aussi appliquer \plm=count= à une instance de tableau.
