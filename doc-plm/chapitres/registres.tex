...
register ICSR at 0xE000_ED04 $uint32 {
  ...
  ISRPREEMPT, ISRPENDING, 1, VECTPENDING[9], RETTOBASE, 2,
  VECTACTIVE[9]
}
\end{PLM}
...
\plm+ICSR.VECTPENDING+ applique un masquage de la valeur de \plm+ICSR+ pour ne conserver que les bits correspondants au champ \plm+VECTPENDING+. Aucun décalage n'est effectué, la valeur renvoyée est une valeur $x\centerdot 2^{12}$, où $x$ est compris en $0$ et $2^9-1$.


Pour obtenir une valeur justifiée à droite (la valeur renvoyée est alors comprise en $0$ et $2^9-1$), on ajoute l'accesseur \plm+.shifted+ :
\begin{PLM}
let x $uint32 = ICSR.VECTPENDING.shifted // Entre 0 et 2**9-1
\end{PLM}
...
Une construction particulière permet de définir facilement la valeur à affecter à un registre de contrôle. Prenons l'exemple du regitre \texttt{ISCR} (\refFigurePage{}{definitionICSR}), et supposons que l'on veuille …
...
\item si \plm+x+ est une expression non calculable statiquement :
  \begin{itemize}
  \item si \plm+x+ est du type \plm+$uint9+, alors toute valeur de \plm+x+ est acceptable : le code engendré se borne à faire le décalage à gauche de la valeur de \plm+x+ ;
...
À chaque champ booléen d'un registre est associée une constante. Par exemple, pour activer l'interruption \texttt{PendSV}, il faut mettre à $1$ le bit \texttt{PENDSVSET}. On écrit donc :
\begin{PLM}
ICSR = $ICSR {PENDSVSET}
\end{PLM}

La constante \plm+$ICSR.PENDSVSET+ a le type du registre \plm+ICSR+, c'est-à-dire \plm+$uint32+, et pour valeur $2^{28}$, le bit \texttt{PENDSVSET} étant le $28^e$ bit du registre \plm+ICSR+.
