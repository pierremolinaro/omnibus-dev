...
À titre d'exemple, nous allons nous intéresser au registre \texttt{ICSR} du processeur ARMv7-M. Le \emph{manuel de référence de l'architecture ARMv7-M}\footnote{\url{http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.ddi0403e.b/index.html}} décrit ce registre comme indiqué à la \refFigure{}{definitionICSR}. Le manuel de référence indique par ailleurs que l'adresse de ce registre est \texttt{0xE000ED04}.

\begin{figure}[t]
\centering
\includegraphics{chapitres/icsr-armv7m.pdf}
...
Le délimiteur \plm+::+ permet de simplifier l'écriture d'une valeur correspondant à un champ entier. Par exemple :
...
\item si \plm+x+ est une expression non calculable statiquement :
  ...
  \item sinon, une assertion (code : voir \refTableauPage{tableauCodeExceptions}) est engendrée ; la valeur de \plm+x+ est ensuite masquée pour pallier un éventuel débordement, puis décalé. 
  \end{itemize}
\end{itemize}
...
\plm+ICSR.VECTPENDING+ applique un masquage de la valeur de \plm+ICSR+ pour ne conserver que les bits correspondants au champ \plm+VECTPENDING+. Aucun déclage n'est effectué.
