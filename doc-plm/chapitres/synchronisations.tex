...
@noUnusedWarning @mutating (?untilDeadline: inDeadline $uint32
                            !result: outResult $bool) {
  outResult = self.value > 0
  if outResult {
    self.value -= 1
  }else if inDeadline > time.now () { 
    block (!?inList:self.list !onDeadline:inDeadline !?result:outResult)
  }
}
\end{PLM}
...
\begin{itemize}
  ...
  \item la valeur de l'argument \plm=inDeadline=~; si l'échéance est atteinte, la primitive retourne immédiatement avec \plm=outResult= à \plm=no=.
\end{itemize}
